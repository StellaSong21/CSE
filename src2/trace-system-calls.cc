...
const string& trim(string s) {
  s.erase(s.begin(), find_if(s.begin(), s.end(), [](int ch) {
    return !isspace(ch); // 1, not a space; 
  })); // 去除前面的所有空格
  s.erase(find_if(s.rbegin(), s.rend(), [](int ch) {
    return !isspace(ch);
  }).base(), s.end()); // 去除最后面的所有空格
  const string& x = s;
  return x;
}
...
static const string kUniversalStandardAbsoluteFilename = "./unistd_64.h";
 ...
 */
static const string kSystemCallNameAndSignaturePattern = 
"\\s*SYSCALL_DEFINE([0-6])\\s*\\(([^,)]+).*";
static pair<string, int> processNameAndArgumentCount(const string& macro, map<string, 
  systemCallSignature>& systemCallSignatures) {
...
static void processSystemCallArguments(const string& macro, int numArguments, systemCallSignature& parameterTypes) {
  ...
  string pattern = "[^(]+\\([^,]+"; 
  // skip everything up through and including macro name, open parenthesis, and first token
 ...
 */
static void processSystemCallSignature(const string& macro, 
  ...
  if (systemCallNames.find(name) == systemCallNames.cend() || // cend返回最后一个元素的下一个地址
      systemCallSignatures.find(name) != systemCallSignatures.cend()) return; 
      // either don't know the system call or we've already processed it
 ...
 */
static void processAllKernelSourceFiles(const subprocess_t& sp, map<string, 
  systemCallSignature>& systemCallSignatures, 
  const map<string, int>& systemCallNames) {
...
static void collectSystemCallSignatures(map<string, systemCallSignature>& systemCallSignatures, 
  const map<string, int>& systemCallNames, 
  bool rebuild) {
